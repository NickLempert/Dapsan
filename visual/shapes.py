...
from visual.effects import Effect, Solid, Lined, Empty
...
class Circle(Shape):
    ...
    def get_value(self, x: float, y: float) -> bool:
        ...
        if abs(dist_dif) <= 1 and self.edge_type.get_value(dist_dif, get_angle(*local_pos)*self.get_radius()/math.pi*3):
            return True
    ...
    def to_mesh(self, quality=20) -> Mesh:
        out = Mesh([],
